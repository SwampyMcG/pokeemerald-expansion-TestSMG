...
#define SOUND_LIST_BGM              \
    ...
    X(MUS_THE_FINAL_COUNTDOWN_EUROPE) \
    X(MUS_WONDERWALL_OASIS_FULL) \
    X(MUS_OLIVER_TREE_LIFE_GOES_ON) \
    X(MUS_OLIVER_TREE_F_K_THE_WHOLE_WORLD) \
    X(MUS_COWBOYS_DONT_CRY_BY_OLIVER_TREE) \
    X(MUS_KAVINSKY_NIGHTCALL) \
    X(MUS_LAGWAGON_VIOLINS_2_WWW_MIDISFREE_COM) \
    X(MUS_ALKALINE_TRIO_MERCY_ME_MIDIFIND_COM) \
    X(MUS_ALKALINE_TRIO_TIME_TO_WASTE_MIDIFIND_COM) \
    X(MUS_ALKALINE_TRIO_RADIO_2_MIDIFIND_COM) \
    X(MUS_ACDC_HIGHWAY_TO_HELL_MIDIFIND_COM) \
